...
const LEAD_STATUS_BADGE: Record<string, string> = {
  new:       'bg-[#F3F7F3] text-[#315C4A] border-[#E8F0EA]',
  contacted: 'bg-[#A8BDAF]/30 text-[#315C4A] border-[#A8BDAF]',
  qualified: 'bg-[#315C4A]/10 text-[#315C4A] border-[#718078]/30',
  proposal:  'bg-[#315C4A]/20 text-[#315C4A] border-[#315C4A]/40',
  won:       'bg-[#315C4A] text-white border-[#315C4A]',
  lost:      'bg-[#718078]/10 text-[#718078] border-[#718078]/30',
}

const PROJECT_STATUS_BADGE: Record<string, string> = {
  active:    'bg-[#315C4A]/10 text-[#315C4A] border-[#A8BDAF]',
  planning:  'bg-[#F3F7F3] text-[#718078] border-[#E8F0EA]',
  on_hold:   'bg-[#E8F0EA]/60 text-[#718078] border-[#E8F0EA]',
  completed: 'bg-[#A8BDAF]/40 text-[#315C4A] border-[#A8BDAF]',
  cancelled: 'bg-[#718078]/10 text-[#718078] border-[#718078]/30',
}

const STAT_ITEMS = [
  { icon: DollarSign, label: 'Business Revenue', bg: 'bg-[#F3F7F3] text-[#315C4A]' },
  { icon: Target,     label: 'Pipeline Value',   bg: 'bg-[#A8BDAF]/30 text-[#315C4A]' },
  { icon: Users,      label: 'Active Clients',   bg: 'bg-[#E8F0EA]/50 text-[#718078]' },
  { icon: Briefcase,  label: 'Active Projects',  bg: 'bg-[#F3F7F3] text-[#718078]' },
]
...
export function BusinessPage() {
  ...
  const emptyCard = (msg: string) => (
    <div className="bg-white rounded-2xl border border-[#E8F0EA] p-8 text-center text-sm text-[#718078]">{msg}</div>
  )
  ...
  return (
    <div className="p-4 md:p-6 lg:p-8 max-w-5xl mx-auto space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 pb-4 border-b border-[#E8F0EA]">
        <div>
          <h1 className="text-2xl font-black text-[#26352E] tracking-tight">Your Business</h1>
          <p className="text-sm text-[#718078] mt-0.5">CRM, projects, pipeline & revenue</p>
        </div>
      ...
      {isLoading ? (
        <div className="text-center py-12 text-[#718078] text-sm">Loading business dashboard…</div>
      ) : (
        <>
          {/* OVERVIEW */}
          {activeTab === 'overview' && (
            <div className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                {STAT_ITEMS.map(({ icon: Icon, label, bg }, i) => (
                  <div key={label} className="bg-white rounded-2xl border border-[#E8F0EA] p-5">
                    ...
                    <div className="text-2xl font-black text-[#26352E]">{statValues[i]}</div>
                  </div>
                ))}
              </div>

              <div className="space-y-3">
                <h3 className="font-bold text-base text-[#26352E]">Client Roster</h3>
                {clients.length > 0 ? (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {clients.map((c: Client) => (
                      <div key={c.id} className="bg-white rounded-2xl border border-[#E8F0EA] p-4">
                        <h4 className="font-semibold text-[#26352E]">{c.name}</h4>
                        <p className="text-xs text-[#718078]">{c.company ?? 'Individual Client'}</p>
                        {c.email && <p className="text-xs text-[#315C4A] font-medium mt-0.5">{c.email}</p>}
                      </div>
                    ))}
                  </div>
            ...
            <div className="space-y-4">
              {clients.length > 0 ? (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {clients.map((c: Client) => (
                    <div key={c.id} className="bg-white rounded-2xl border border-[#E8F0EA] p-5">
                      <h4 className="font-bold text-base text-[#26352E]">{c.name}</h4>
                      {c.company && <p className="text-xs font-semibold text-[#718078] mt-0.5">{c.company}</p>}
                      <div className="mt-3 space-y-1 text-xs text-[#718078]">
                        {c.email && <div>Email: <span className="text-[#315C4A]">{c.email}</span></div>}
                        {c.phone && <div>Phone: {c.phone}</div>}
                        {c.notes && <div className="mt-2 pt-2 border-t border-[#F3F7F3] text-[#26352E]">{c.notes}</div>}
                      </div>
                    </div>
                  ))}
                </div>
          ...
          {activeTab === 'leads' && (
            <div className="space-y-3">
              {leads.length > 0 ? leads.map((l: Lead) => (
                <div key={l.id} className="bg-white rounded-2xl border border-[#E8F0EA] p-4 flex items-center justify-between">
                  <div>
                    <div className="flex items-center gap-2">
                      <h4 className="font-semibold text-[#26352E]">{l.name}</h4>
                  ...
                  <select
                    ...
                    className="rounded-lg border border-[#E8F0EA] bg-[#F3F7F3] px-2 py-1 text-xs font-semibold text-[#315C4A] focus:outline-none focus:ring-2 focus:ring-[#718078]"
                  >
              ...
              {projects.length > 0 ? (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {projects.map((p: BusinessProject) => (
                    <div key={p.id} className="bg-white rounded-2xl border border-[#E8F0EA] p-5">
                      <div className="flex justify-between items-start mb-2">
                        <h4 className="font-semibold text-[#26352E]">{p.title}</h4>
                      ...
                      {p.description && <p className="text-xs text-[#718078] mt-1">{p.description}</p>}
                    </div>
                  ))}
                </div>
